...
public final class SellBookGUI extends javax.swing.JFrame {
    ...
    private void updateBuyTable(String ma, int soLuong, long thanhTien) {
        DefaultTableModel modelBuy = (DefaultTableModel) buyTable.getModel();
        
        int row = modelBuy.getRowCount();
        
        for (int i = 0; i < row; i++) {
            if (modelBuy.getValueAt(i, 0).equals(ma)) {
    ...
    private void updateTotalPay() {
        ...
        int row = modelBuy.getRowCount();
        for (int i = 0; i < row; i++) {
            long pay = (long) modelBuy.getValueAt(i, 4);
            total += pay;
        }
        
        totalPay.setText(nf.format(total) + " VNĐ");
    }
    ...
    private void addEventBookTable() {
        ...
        bookTable.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ...
                if (row >= 0 && col == 4) {
                    String ma =  String.valueOf(bookTable.getValueAt(row, 0));
    ...
    private void showComfirmRemoveAll(int i, int row) {
        ...
        if (JOptionPane.showConfirmDialog(this, "Bạn chắc chứ?", "Question", 2) == 0) {
            buyList.remove(i);
            String ma = (String) modelBuy.getValueAt(row, 0);
        ...
        buyTable.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ...
                if (row >= 0 && col == 5) {
                    ...
                    for (int i = 0; i < buyList.size(); i++) {
                        if (Array.get(buyList.get(i), 0).toString().equals(ma)) {
                            String maSach = Array.get(buyList.get(i), 0).toString();
    ...
    private void payBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_payBtnMouseClicked
        ...
        String uniqueID = UUID.randomUUID().toString().split("-")[2];
        PhieuBanDTO pb = new PhieuBanDTO("PB" + uniqueID, kh.getMaKhachHang(), tk.getMaNhanVien(), ngayLap);
        
        phieuBanBLL.insert(pb);
        
        ChiTietPhieuBanDTO ctpb;
        
        for (Object[] buy : buyList) {
            ma = String.valueOf(Array.get(buy, 0));
            soLuongMua = Integer.parseInt(Array.get(buy, 3).toString());
            s = sachBLL.getById(ma);
            s.setSoLuongConLai(s.getSoLuongConLai() - soLuongMua);
            sachBLL.update(s, ma);
            
            ctpb = new ChiTietPhieuBanDTO(pb.getMaPhieuBan(), ma, soLuongMua, s.getGiaBan());
    ...
    private void addCustomerBtnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_addCustomerBtnMouseClicked
        ...
        if (result == JOptionPane.OK_OPTION) {
            ...
            String uniqueID = UUID.randomUUID().toString().split("-")[1];
            
            this.kh = new KhachHangDTO("KH" + uniqueID, ten, gioiTinh, sdt, namNumber, 0);
